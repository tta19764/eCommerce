...
public sealed class SellerCommandHandlerTests
{
    ...
    public async Task Submit_ShouldAddPendingSellerAndNormalizedStore()
    {
        ...
        var handler = new SubmitSellerApplicationCommandHandler(_repository, _unitOfWork);
        ...
        _repository.Received(1).Add(Arg.Is<Store>(store =>
    ...
    public async Task Submit_ShouldRejectExistingOwnerWithoutSaving()
    {
        ...
        result.Error.Should().Be(SellerApplicationErrors.AlreadyExists);
    ...
    public async Task Approve_ShouldActivateSellerAndSave()
    {
        ...
        var handler = new ApproveSellerCommandHandler(_repository, _unitOfWork);
    ...
    {
        var handler = new RejectSellerCommandHandler(_repository, _unitOfWork);

        var result = await handler.Handle(
            new RejectSellerCommand(Guid.NewGuid(), Guid.NewGuid(), "The application is incomplete."),
            CancellationToken.None);

        result.Error.Should().Be(SellerApplicationErrors.NotFound);
        await _unitOfWork.DidNotReceive().SaveChangesAsync(Arg.Any<CancellationToken>());
    }
}
