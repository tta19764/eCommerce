...
public sealed class IntegrationTestWebAppFactory : IAsyncLifetime
{
                ...
                return Task.FromResult<Response<GetProductDetailsResponse>>(
                    new TestResponse<GetProductDetailsResponse>(
                        _products.GetValueOrDefault(
                            request.ProductId,
                            new GetProductDetailsResponse(request.ProductId, string.Empty, string.Empty, 0m, "USD", 0, null, 0.0m, 0, false))));
            });
    ...
    {
        _products[productId] = new GetProductDetailsResponse(productId, name, string.Empty, price, currencyCode, quantity, null, 0.0m, 0, true);
    }
